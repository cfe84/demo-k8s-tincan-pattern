...
# Start the fulfillment service and its facade
bounce -P $FULFILLMENT_SERVICE_PORT -p '/api/fulfillment-order' -r 'Fulfillment created' &
PROXY_TO=http://127.0.0.1:$FULFILLMENT_SERVICE_PORT PORT=$FULFILLMENT_SERVICE_FACADE_PORT AUDIENCE=aud:fulfillment-service node ./authFacade/server.js &
